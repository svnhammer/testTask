...
import { BINANCE_BASE_URL, TREND_FALLING, TREND_RISING, TREND_STABLE } from './trades.consts';
...
export class TradesService {
    ...
    async getHistoricalTrades(symbol: string, startTime?: number, endTime?: number): Promise<Record<string, any>[]> {
        const response = await axios.get(`${BINANCE_BASE_URL}aggTrades`, {
            params: { symbol: symbol.toUpperCase(), startTime: startTime * 1000, endTime: endTime * 1000 }
        });
    ...
    async analyzePriceChanges(symbol: string, startTime: number, endTime: number) {
        const retrievedTrades = await this.tradeEntryModel.find({ symbol, timestamp: { $gte: startTime, $lte: endTime } }).sort({ time: 1 }).exec();

        if (retrievedTrades.length < 2) {
            throw new InternalServerErrorException('Insufficient trades stored for given time period');
        }

        const mostRecent = retrievedTrades[0];
        const leastRecent = retrievedTrades[retrievedTrades.length - 1];

        const priceChange = parseFloat(mostRecent.price) - parseFloat(leastRecent.price);

        const percentChange = ((priceChange / parseFloat(leastRecent.price)) * 100).toFixed(4);
        const trend = priceChange > 0 ? TREND_RISING : priceChange < 0 ? TREND_FALLING : TREND_STABLE
